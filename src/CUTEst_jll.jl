...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import CUTEst_jll on Julia versions older than 1.3!")
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("bb5f6f25-f23d-57fd-8f90-3ef7bad1d825"))
platforms = [Pkg.Artifacts.unpack_platform(e, "CUTEst", artifacts_toml) for e in artifacts["CUTEst"]]
...
if best_platform === nothing
    @debug("Unable to load CUTEst; unsupported platform $(triplet(platform_key_abi()))")
else
